		...
		*/
		...
		/*
		    ...
		    minNum      可设置为内容的最小高度        类型 Number
		                如果计算出来的值大于该值则使用计算的值
		*/
		winChange:function (obj, minNum, hideNum) {
		...
		uploadToast:function (isSuccess) {
			var box_content = '';
			if(isSuccess) {
		...
		//评论的展开与隐藏
		showComment:function (obj, target) {
			var $li = $(target, obj);
			if($li.length > 5) {
		...
		//把search值转化成对象
		getQueryObj:function () { //参数为window.location.search
