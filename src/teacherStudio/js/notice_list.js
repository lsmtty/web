...
function initData() {
	pageNotice = getPagingBar(updateData);
	updateData(0);
	if(canPublish()) {
...
function updateData(i) {
	...
	var pagingURL = studioHost + "notice/queryPage/" + titleID + "/" + PAGE_COUNT + "/" + (i + 1);
	$.ajax({
		type: 'get',
		url: pagingURL
	}).success(
		function(data) {
			...
			if(Math.ceil(data.page.totalCount / PAGE_COUNT) != pageNotice.getPageSize()) {
				pageNotice.reloadPage(Math.ceil(data.page.totalCount / PAGE_COUNT));
			}
		});
}
	...
	if(ItemArray.length == 0) {
		...
		for(var index in ItemArray) {
			var articleString = '<li class="news-list-item" data-id="' + ItemArray[index].id + '" data-creater="' + ItemArray[index].creater + '">' +
				'<span class="article-name"><a href="' + 'notice_details.html?titleID=' + ItemArray[index].id + '&studioID=' + getQueryObj().studioID + '">' + ItemArray[index].title +
				'</a></span><span class="article-author"></span><span class="news-publish-time">' +
				ItemArray[index].createTime + '</span><div class="delete-area" onclick="deleteItem(this);"><div class="icon-delete"></div><span>删除</span></div></li>';
...
function deleteItem(item) {
	...
	if(canDelete($(item).parent().data("creater"))) {
		...
		deleteDialog(function() {
			var deleteurl = studioHost + "/notice/del/" + itemID;
			$.ajax({
				type: "delete",
				url: deleteurl
